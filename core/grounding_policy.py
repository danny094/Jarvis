...
def _normalize(policy: Dict[str, Any]) -> Dict[str, Any]:
    ...
    stream_postcheck_mode = str(
        output.get("stream_postcheck_mode", "tail_repair")
    ).strip().lower()
    output["stream_postcheck_mode"] = (
        stream_postcheck_mode
        if stream_postcheck_mode in {"tail_repair", "buffered", "off"}
        else "tail_repair"
    )
