...
def test_fallback_skipped_for_container_domain_locked_runtime_turn():
    cm = _make_cm()
    thinking_plan = {
        "needs_memory": True,
        "memory_keys": [],
        "is_fact_query": False,
        "_domain_route": {"domain_tag": "CONTAINER", "domain_locked": True, "operation": "exec"},
    }
    called_with_keys = {"called": False}
    ...
    try:
        with patch.object(cm, "_load_trion_laws", return_value=""), \
             ...
             patch.object(cm, "_search_memory_keys_parallel", side_effect=fake_parallel):
            cm.get_context(
                query="Finde die Host-IP und nutze Runtime-Tools",
                thinking_plan=thinking_plan,
                conversation_id="conv-container-runtime",
                small_model_mode=True,
            )
    finally:
        for p in patches:
    ...
    assert called_with_keys["called"] is False


def test_fallback_skipped_for_non_recall_turn_with_empty_keys():
    cm = _make_cm()
    thinking_plan = {
        "needs_memory": True,
        "memory_keys": [],
    ...
    try:
        with patch.object(cm, "_load_trion_laws", return_value=""), \
             ...
             patch.object(cm, "_search_memory_keys_parallel", side_effect=fake_parallel):
            cm.get_context(
                query="Fuehre den Runtime-Statuscheck aus",
                thinking_plan=thinking_plan,
                conversation_id="conv-runtime-no-recall",
                small_model_mode=True,
            )
    finally:
        for p in patches:
