...
async def upload_persona(file: UploadFile = File(...)) -> Dict[str, Any]:
    ...
    try:
        ...
        name = file.filename[:-4]  # Remove '.txt'
        
        # Validate name
        if not _validate_persona_name(name):
            raise HTTPException(
                status_code=400,
                detail="Invalid filename. Use alphanumeric, dash, underscore only."
            )
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to upload persona: {str(e)}"
        )
...
async def delete_persona_endpoint(name: str) -> Dict[str, Any]:
    ...
    Protection: Cannot delete 'default' persona.
    ...
    Status Codes:
        200: Success
        400: Invalid name or trying to delete 'default'
        404: Persona not found
        500: Internal server error
    """
    # Validate name
